...
//		}
//		for (int i = 0; i < words.length; i++) {
//			dfs(result, dict, words, i, words[i]);
//		}
//		return result;
//	}
//	
//	private void dfs(List<String> result, Map<Integer, Set<Integer>> dict, String[] words, int i, String w) {
//		if (w.length() == 0) result.add(words[i]);
//		for (int len : dict.keySet()) {
//			if (w.length() >= len) {
//				for (int idx : dict.get(len)) {
//					if (idx != i && w.startsWith(words[idx]))
//						dfs(result, dict, words, i, w.substring(len));
//				}
//			}
//		}
//	}
	...
	public List<String> findAllConcatenatedWordsInADict(String[] words) {
		...
		for (int i = 0; i < words.length; i++) {
			if (searchWord(root, words[i], true))
				result.add(words[i]);
		}
		return result;
	}
	...
	private boolean searchWord(TrieNode root, String word, boolean start) {
		TrieNode node = root;
		for (int i = 0; i < word.length(); i++) {
			char c = word.charAt(i);
			...
			node = node.children[c - 'a'];
			if (node.word != null) { 
				if (!start && node.word.equals(word)) return true;
				if (searchWord(root, word.substring(i + 1), false)) return true;
			}
		}
		return false;
	}
...
//		String[] words = {"cat","cats","catsdogcats","dog","dogcatsdog","hippopotamuses","rat","ratcatdogcat"};
		...
		for (String w : cw.findAllConcatenatedWordsInADict(words)) {
